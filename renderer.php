...
/**
 * Hybrid question renderer class.
 *
 * @package    qtype
 * @subpackage hybrid
...
/**
 * Generates the output for hybrid questions.
 *
 * @copyright  2009 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_hybrid_renderer extends qtype_renderer {
    public function formulation_and_controls(question_attempt $qa,
        ...
        $result = '';
        ////////////////////////////////////////////////
        // QRMOOD-46 - As a student, I want clearer question when I upload my files.
        //
        // Add the question name only if we are in the mobile section.
        if ($this->page->pagetype == 'local-qrsub-attempt') {
            $result .= html_writer::tag('h2', $question->get_question_name());

            // Display the question description.
            $result .= html_writer::tag(
                'div',
                $question->format_questiontext($qa),
                array('class' => 'qtext collapse', 'id' => 'question_desc', 'aria-expanded' => 'false')
            );

            // Add the "Show more" link.
            $result .= html_writer::tag('a', '',
            array(
                'role' => 'button', 'class' => 'collapsed', 'data-toggle' => 'collapse',
                'href' => '#question_desc', 'aria-expanded' => 'false', 'aria-controls' => 'question_desc',
                'id' => 'question_desc_link'
                )
            );
        } else {
        ...
        $this->page->requires->js_call_amd(
            'qtype_hybrid/response_summary_text',
            'init',
            array($default_msg)
        );
        // QRMOOD-41
    ...
    public function files_input(question_attempt $qa, $numallowed,
        ...
        if (!empty($qa->get_question()->filetypeslist)) {
            $text = html_writer::tag('p', get_string('acceptedfiletypes', 'qtype_hybrid'));
    ...
    public function manual_comment(question_attempt $qa, question_display_options $options) {
        ...
        return html_writer::nonempty_tag('div', $question->format_text(
                $question->graderinfo, $question->graderinfo, $qa, 'qtype_hybrid',
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_hybrid_format_renderer_base extends plugin_renderer_base {
    /**
...
/**
 * An hybrid format renderer for hybrids where the student should not enter
 * any inline response.
 *
 * @copyright  2013 Binghamton University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_hybrid_format_noinline_renderer extends plugin_renderer_base {

    protected function class_name() {
        return 'qtype_hybrid_noinline';
    }
...
/**
 * An hybrid format renderer for hybrids where the student should use the HTML
 * editor without the file picker.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_hybrid_format_editor_renderer extends plugin_renderer_base {
    protected function class_name() {
        return 'qtype_hybrid_editor';
    }

    public function response_area_read_only($name, $qa, $step, $lines, $context) {
        return html_writer::tag('div', $this->prepare_response($name, $qa, $step, $context),
                ['class' => $this->class_name() . ' qtype_hybrid_response readonly',
    ...
    public function response_area_input($name, $qa, $step, $lines, $context) {
        ...
        $output .= html_writer::start_tag('div', array('class' =>
                $this->class_name() . ' qtype_hybrid_response'));
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_hybrid_format_editorfilepicker_renderer extends qtype_hybrid_format_editor_renderer {
    protected function class_name() {
        return 'qtype_hybrid_editorfilepicker';
    }
    ...
    protected function specific_filepicker_options($acceptedtypes, $draftitemid, $context) {
        debugging('qtype_hybrid_format_editorfilepicker_renderer::specific_filepicker_options() is deprecated, ' .
...
/**
 * An hybrid format renderer for hybrids where the student should use a plain
 * input box, but with a normal, proportional font.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_hybrid_format_plain_renderer extends plugin_renderer_base {
    /**
     * @return string the HTML for the textarea.
     */
    protected function textarea($response, $lines, $attributes) {
        $attributes['class'] = $this->class_name() . ' qtype_hybrid_response form-control';
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_hybrid_format_monospaced_renderer extends qtype_hybrid_format_plain_renderer {
    protected function class_name() {
        return 'qtype_hybrid_monospaced';
    }
}
